...
public record Password : ValueObject
{
    ...
    private const int DefaultExpirationInMinutes = 120;
    ...
    private Password(string hash,DateTime? expiresAtUtc,IDateTimeProvider provider)
    {
        Hash = hash;
    ...
    public static Password ShouldCreate(string plainText) => ShouldCreate(plainText, provider:null);
    public static Password ShouldCreate(string plainText,IDateTimeProvider? provider)
    {
        ...
        return new Password(hash,expiresAtUtc: provider.UtcNow.Add(TimeSpan.FromMinutes(DefaultExpirationInMinutes)),provider:provider);
    }
    ...
    {
        return ExpiresAtUtc is null || ExpiresAtUtc < _provider.UtcNow;
    } 
